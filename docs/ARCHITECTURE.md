...
|  bot/web_server.py — aiohttp dashboard server (port 8080)    |
|  bot/html_report.py — HTML dashboard renderer (Chart.js)     |
+-------------------------------------------------------------+
                          |
                          v
+-------------------------------------------------------------+
...
| Scheduler | `scheduler.py` | Background jobs: attendance check (15m), daily check (22:07), session keepalive (15m) |
| Web Server | `web_server.py` | aiohttp server for HTML dashboard (port 8080) |
| HTML Report | `html_report.py` | Dashboard renderer with Chart.js (dark theme, responsive) |
...
| Status Commands | `commands/status.py` | `/status`, `/ping`, `/daily` |
...
| Daily Check | Daily 22:07 | Fetch grades/schedule/exams/attendance for all courses, render HTML dashboard, compare with snapshot, notify changes |
| Session Keepalive | Every 15 min | Check session health, trigger re-login if expired |
...
| Daily Report | `data/daily_report.html` | HTML file | Auto-generated by daily check |
| Snapshot | `data/snapshots/weekly_snapshot.json` | JSON | Auto-generated by daily check |
