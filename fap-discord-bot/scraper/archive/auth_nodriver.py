...
class FAPAuthNodriver:
    ...
    async def _check_session_valid(self) -> bool:
        ...
        try:
            tab = await self._browser.get(self.SCHEDULE_URL)
    ...
    async def _full_login_flow(self) -> bool:
        ...
        try:
            ...
            tab = await self._browser.get(self.LOGIN_URL)
            self._tab = tab
            ...
            # Find username input using nodriver's query_selector
            username_input = await tab.query_selector('input[name="ctl00$mainContent$UserName"]')
            if not username_input:
                logger.error("Username input not found!")
                # Try alternative selector
                username_input = await tab.query_selector('#ctl00_mainContent_UserName')
            ...
            # Find password input
            password_input = await tab.query_selector('input[name="ctl00$mainContent$Password"]')
            if not password_input:
                password_input = await tab.query_selector('#ctl00_mainContent_Password')
            ...
            login_btn = await tab.query_selector('#ctl00_mainContent_btnLogin')
            if login_btn:
            ...
            if 'Login.aspx' not in current_url and 'Default.aspx' in current_url:
