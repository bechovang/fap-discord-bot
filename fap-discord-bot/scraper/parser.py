...
class FAPParser:
    ...
    def parse_schedule(self, html_content: str) -> List[ScheduleItem]:
        ...
        try:
            ...
            # Find schedule table - look for table with Slot rows
            table = None
            for t in soup.find_all('table'):
                ...
                if tbody:
                    ...
                    if rows:
                        ...
                        if first_cell and 'Slot' in first_cell.get_text():
                            table = t
                            logger.info(f"Found schedule table with {len(rows)} rows")
                            break
            ...
            if tbody:
                rows = tbody.find_all('tr')
                logger.info(f"Processing {len(rows)} slot rows")
                for row in rows:
    ...
    def get_today_schedule(self, items: List[ScheduleItem], now: datetime = None) -> List[ScheduleItem]:
        """Get schedule items for today by matching weekday label.

        FAP's date numbers in schedule headers are often off by 1 day from
        the real calendar, but the weekday labels (Mon, Tue, ...) are correct.
        So we match by weekday label, not by date string.
        """
        if now is None:
            now = datetime.now()
        today_map = {
